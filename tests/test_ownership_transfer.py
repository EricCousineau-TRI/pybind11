...
def test_shared_ptr_derived_slicing(capture):
    ...
    del obj
    assert cstats.alive() == 1
    # We now still have a reference to the object. py::alias_wrapper<> will intercept Python's
    # attempt to destroy `obj`, is aware the `shared_ptr<>.use_count() > 1`, and will increase
    # the ref count by transferring a new reference to `py::alias_wrapper<>` (thus reviving the object,
    ...
    # py::alias_wrapper<> in C++ should have released its reference.
